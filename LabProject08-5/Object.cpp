...
void CMaterial::UpdateShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, int nParameterIndex, int nTextureIndex)
{
	if (m_pTexture) m_pTexture->UpdateShaderVariable(pd3dCommandList, nParameterIndex, nTextureIndex);
}
...
CSkyBox::CSkyBox(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(6)
{
	...
	CScene::CreateShaderResourceViews(pd3dDevice, pSkyBoxTexture, 0, 6);
...
void CSkyBox::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_ppMeshes)
	{
		for (int i = 0; i < m_nMeshes; i++)
		{
			if (m_pMaterial)
			{
				m_pMaterial->UpdateShaderVariable(pd3dCommandList, 0, i);
			}
			if (m_ppMeshes[i]) m_ppMeshes[i]->Render(pd3dCommandList);
		}
	}
